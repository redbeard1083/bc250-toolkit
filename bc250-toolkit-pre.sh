...
# AUR helper — detects paru or yay in that order
aur_helper() {
...
# Installs paru automatically if no AUR helper is present. Tries pacman
# first (CachyOS ships paru directly in its own repos), falling back to
...
ensure_aur_helper() {
    ...
    if command -v pacman >/dev/null 2>&1 && pacman -Si paru >/dev/null 2>&1; then
        if pacman -S --needed --noconfirm paru; then
            aur_helper >/dev/null 2>&1 && return 0
        fi
        print_error "pacman could not install paru — check the output above."
        return 1
    fi

    print_info "paru is not available via pacman — building it from the AUR..."
    local user_home build_dir
    user_home="$(getent passwd "$REAL_USER" | cut -d: -f6)"
    build_dir="$user_home/.cache/bc250-toolkit/paru"

    sudo -u "$REAL_USER" mkdir -p "$(dirname "$build_dir")"
    if [[ -d "$build_dir" ]]; then
        sudo -u "$REAL_USER" git -C "$build_dir" pull || { print_error "Failed to pull the paru repository."; return 1; }
    else
        sudo -u "$REAL_USER" git clone https://aur.archlinux.org/paru.git "$build_dir" || { print_error "Failed to clone the paru repository."; return 1; }
    fi

    if ! sudo -u "$REAL_USER" bash -c "cd '$build_dir' && makepkg -si --noconfirm"; then
        print_error "Failed to build/install paru from source."
        return 1
    fi
    ...
    print_error "paru installation reported success but the binary could not be found afterward."
    return 1
}
...
aur_install() {
    ...
    if ! ensure_aur_helper; then
        print_error "No AUR helper available and automatic installation of paru failed."
        return 1
    fi
    local helper
    helper="$(aur_helper)"
